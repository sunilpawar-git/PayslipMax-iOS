...
//
// `CGPDFScanner` invokes C-convention callbacks; the mutable `OfficerColumnarProbe.Scan`
// is recovered from the opaque `info` pointer. Names mirror the PDF operators they
// implement. Split out of `OfficerColumnarProbe.swift` to keep each file under the limit.

private func scanState(_ info: UnsafeMutableRawPointer?) -> OfficerColumnarProbe.Scan {
    guard let info else {
        preconditionFailure("CGPDFScanner invoked without info pointer")
    }
    return Unmanaged<OfficerColumnarProbe.Scan>.fromOpaque(info).takeUnretainedValue()
}
...
private func moveLine(_ scan: OfficerColumnarProbe.Scan, _ tx: CGFloat, _ ty: CGFloat) {
...
enum PDFTextOps {
    ...
    private static func runForm(
        _ scan: OfficerColumnarProbe.Scan,
