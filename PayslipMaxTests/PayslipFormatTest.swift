...
final class PayslipFormatTest: XCTestCase {
    ...
    func testDefensePayslipFormatCases() {
        ...
        let defenseFormats: [PayslipFormat] = [.defense, .jcoOR, .unknown]

        XCTAssertEqual(defenseFormats.count, 3)
        XCTAssertTrue(defenseFormats.contains(.defense))
        XCTAssertTrue(defenseFormats.contains(.jcoOR))
    ...
    func testDefensePayslipFormatSwitching() {
        ...
        let formats = [PayslipFormat.defense, PayslipFormat.jcoOR, PayslipFormat.unknown]
        ...
        for format in formats {
            ...
            case .jcoOR:
                XCTAssertEqual(format, .jcoOR, "JCO/OR case should be matched")
            case .unknown:
