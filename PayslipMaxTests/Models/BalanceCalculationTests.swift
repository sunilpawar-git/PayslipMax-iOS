...
final class BalanceCalculationTests: XCTestCase {
    ...
    func testBalanceCalculation() {
        ...
        XCTAssertEqual(net1, 4000.0, "Standard case balance calculation should be correct")
    ...
    func testEdgeCaseBalances() {
        ...
        XCTAssertEqual(zeroCreditsNet, -1000.0, "Zero credits with deductions should result in negative balance")
        ...
        let largeValuePayslip = PayslipItem(
            ...
            debits: 5.0e14,   // 500 trillion (total deductions)
            dsop: 1.0e14,     // 100 trillion (component of debits)
            tax: 2.0e14,      // 200 trillion (component of debits)
            name: "John Doe",
            accountNumber: "XXXX1234",
            panNumber: "ABCDE1234F"
        )
        let largeValueNet = largeValuePayslip.credits - largeValuePayslip.debits  // Net remittance = credits - debits
        XCTAssertEqual(largeValueNet, 5.0e14, "Large value calculations should maintain precision")
    }
} 