...
struct EnhancedPDFView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> PDFView {
        ...
        // Apply the document
        let document = context.coordinator.updatePDFDocument()
        pdfView.document = document
        hasError = (document == nil)
        
        if document == nil {
            context.coordinator.displayErrorMessage(in: pdfView)
        }
    ...
    class Coordinator: NSObject {
        private let parent: EnhancedPDFView
        ...
        func updatePDFDocument() -> PDFDocument? {
            guard let pdfData = parent.pdfData else { return nil }
            ...
            if pdfData.starts(with: pwdMarkerData) {
                ...
                let passwordToUse = extractedPassword ?? parent.password ?? ""
                ...
                // Standard PDF
                document = createDocument(from: pdfData, password: parent.password)
            }
        ...
        func displayErrorMessage(in view: UIView) {
