...
struct ShareSheet: UIViewControllerRepresentable {
    ...
    func makeUIViewController(context: Context) -> UIActivityViewController {
        ...
        // Configure for iPad - use window bounds instead of deprecated UIScreen.main
        if let popoverController = controller.popoverPresentationController {
            popoverController.sourceView = UIView() // Required for iPad

            // Get bounds from the window scene instead of UIScreen.main
            if let windowScene = UIApplication.shared.connectedScenes.first as? UIWindowScene,
               let window = windowScene.windows.first {
                let bounds = window.bounds
                popoverController.sourceRect = CGRect(x: bounds.midX, y: bounds.midY, width: 0, height: 0)
            } else {
                // Fallback to center of a default rect if window is unavailable
                popoverController.sourceRect = CGRect(x: 200, y: 400, width: 0, height: 0)
            }
