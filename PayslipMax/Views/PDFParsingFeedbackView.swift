...
struct PDFParsingFeedbackView: View {
    ...
    private var earningsSection: some View {
        Section(header: Text("Earnings")) {
            ForEach(viewModel.displayEarnings, id: \.displayName) { item in
                if viewModel.isEditing {
                    HStack {
                        Text(item.displayName)
                        Spacer()
                        TextField("Amount", value: Binding<Double>(
                            get: { viewModel.editedEarnings[item.originalKey] ?? 0 },
                            set: { viewModel.editedEarnings[item.originalKey] = $0 }
