...
extension SafeConversionProtocol {
    ...
    @MainActor
    static func resolveInstance() -> Self {
        // Check feature flag to determine which instance to return
        let featureFlagManager = FeatureFlagManager.shared
        let shouldUseDI = featureFlagManager.isEnabled(.dependencyInjectionPhase2)
        
        if shouldUseDI {
            // Try to get DI instance from container
            if let diInstance = DIContainer.shared.resolve(Self.self) {
                return diInstance
            }
        }
