...
enum PayslipTokens {
    static let requiredFields = ["name", "month", "year", "earnings", "deductions"]
    static let nameLabel = "Name:"
    static let monthPattern = "Month:|Date:|Period:"
    static let yearPattern = "Year:|20[0-9]{2}"
    static let earningsTerms = ["Earnings", "Credits", "Salary", "Pay", "Income", "Allowances"]
    static let deductionsTerms = ["Deductions", "Debits", "Tax", "DSOP", "Fund", "Recovery"]
}
...
class PayslipValidationService: PayslipValidationServiceProtocol {
    ...
    func validatePayslipContent(_ text: String) -> PayslipContentValidationResult {
        ...
        let requiredFields = PayslipTokens.requiredFields
        let (detectedFields, missingFields) = detectPayslipFields(in: text)
    ...
    private func detectPayslipFields(in text: String) -> (detected: [String], missing: [String]) {
        var detected: [String] = []
        var missing: [String] = []

        if text.range(of: PayslipTokens.nameLabel, options: .caseInsensitive) != nil {
            detected.append("name")
        } else {
            missing.append("name")
        }

        if text.range(of: PayslipTokens.monthPattern, options: .regularExpression) != nil {
            detected.append("month")
        } else {
            missing.append("month")
        }

        if text.range(of: PayslipTokens.yearPattern, options: .regularExpression) != nil {
            detected.append("year")
        } else {
            missing.append("year")
        }

        let hasEarnings = PayslipTokens.earningsTerms.contains { term in
        ...
        let hasDeductions = PayslipTokens.deductionsTerms.contains { term in
            text.range(of: term, options: .caseInsensitive) != nil
        }
        if hasDeductions {
            detected.append("deductions")
        } else {
            missing.append("deductions")
        }
    ...
    func deepValidatePayslip(_ payslip: any PayslipProtocol) -> PayslipDeepValidationResult {
        ...
        var contentValidation: PayslipContentValidationResult?
