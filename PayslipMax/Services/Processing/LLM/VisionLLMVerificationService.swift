...
final class VisionLLMVerificationService {
    ...
    func verify(
        ...
        if agreement >= 0.8 {
            logger.info("✓ High agreement, using verified result")
            return VisionVerificationResult(response: sanitized, confidence: finalConfidence)
        } else if agreement >= 0.5 {
            logger.info("⚠️ Moderate agreement, averaging results")
            return VisionVerificationResult(response: sanitized, confidence: finalConfidence * 0.9)
        } else {
            logger.warning("❌ Low agreement, using first pass with reduced confidence")
            return VisionVerificationResult(response: firstPassResult, confidence: originalConfidence * 0.8)
        }
    }
    ...
    private func calculateFinalConfidence(originalConfidence: Double, agreement: Double) -> Double {
        if agreement >= 0.9 {
            return min(1.0, originalConfidence + 0.1)
