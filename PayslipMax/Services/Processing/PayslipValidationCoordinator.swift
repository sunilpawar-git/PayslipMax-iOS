...
class PayslipValidationCoordinator: PayslipValidationCoordinatorProtocol {
    ...
    func validateAndGetTotals(
        ...
        if statedGrossPay > 0 {
            ...
            print("[PayslipValidationCoordinator] Credits validation - Extracted: ₹\(Int(extractedCredits)), Stated: ₹\(Int(statedGrossPay)), Variance: \(String(format: "%.1f", creditsVariancePercent))%")

            // If variance exceeds 10%, prefer stated total
            if creditsVariancePercent > 10.0 {
                print("[PayslipValidationCoordinator] WARNING: High variance in credits extraction, using stated total (variance: \(String(format: "%.1f", creditsVariancePercent))%)")
        ...
        if statedTotalDeductions > 0 {
            ...
            print("[PayslipValidationCoordinator] Debits validation - Extracted: ₹\(Int(extractedDebits)), Stated: ₹\(Int(statedTotalDeductions)), Variance: \(String(format: "%.1f", debitsVariancePercent))%")

            if debitsVariancePercent > 10.0 {
                print("[PayslipValidationCoordinator] WARNING: High variance in debits extraction, using stated total (variance: \(String(format: "%.1f", debitsVariancePercent))%)")
