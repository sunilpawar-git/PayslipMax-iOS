            ...
            for match in matches {
                if match.numberOfRanges > 2,
                   ...
                   let amountRange = Range(match.range(at: 2), in: text) {
                    ...
                    if TextExtractionValidation.isBlacklisted(
                        code,
                        in: "earnings",
                        patternProvider: patternProvider
                    ) &&
                    TextExtractionValidation.isBlacklisted(
                        code,
                        in: "deductions",
                        patternProvider: patternProvider
                    ) {
                    ...
                    if let amount = Double(cleaned) {
                        ...
                        if patternProvider.standardEarningsComponents.contains(code) {
                            ...
                            let earningsHint = code.contains("PAY")
                                || code.contains("ALLOW")
                                || code.contains("SALARY")
                                || code.contains("WAGE")
                            let deductionHint = code.contains("TAX")
                                || code.contains("FUND")
                                || code.contains("FEE")
