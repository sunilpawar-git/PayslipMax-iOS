...
class PDFTextExtractionService: PDFTextExtractionServiceProtocol {
    ...
    func extractText(from document: PDFDocument, callback: ((String, Int, Int) -> Void)? = nil) -> String? {
            ...
            autoreleasepool {
                if let page = document.page(at: i), let text = page.string {
                    ...
                    if trackMemoryUsage {
                        ...
                        let pageLabel = "[PDFTextExtractionService] Page \(i + 1)/\(document.pageCount)"
                        let memoryLog = "\(pageLabel): \(formatMemory(memoryUsage)) (Δ\(formatMemory(memoryDelta)))"
                    ...
                    if fullText.lengthOfBytes(using: .utf8) > maxBatchSize {
                        print("[PDFTextExtractionService] Batch limit exceeded; processing intermediate batch")
        ...
        let summaryLog = "[PDFTextExtractionService] Extracted \(fullText.count) chars from \(document.pageCount) pages"
        print(summaryLog)
        return fullText
    }
