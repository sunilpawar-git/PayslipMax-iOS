...
final class PDFCacheCleanup {
    ...
    private func cleanupExpiredItemsInternal() throws {
        ...
        for file in files {
            let resource = try file.resourceValues(forKeys: [.contentModificationDateKey])
            guard let modificationDate = resource.contentModificationDate else { continue }
            if now.timeIntervalSince(modificationDate) > configuration.cacheExpirationTime {
                try FileManager.default.removeItem(at: file)
                expiredCount += 1
            }
        }
