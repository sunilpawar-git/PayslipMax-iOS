        ...
        return await withTaskGroup(of: (Int, String).self) { group in
            ...
            for (index, document) in documents.enumerated() {
                group.addTask {
                    ...
                    let text = await self.processDocumentStreaming(document) { _, _ in }
                    return (index, text)
                }
            }
...
// Note: StreamingProcessingOptions and AsyncSemaphore are defined in other files
// AsyncSemaphore is defined in AsyncPDFProcessingCoordinator.swift
// StreamingProcessingOptions is defined in StreamingTextExtractionService.swift 