...
class FinancialLiteracyQuestionGenerator: QuizQuestionGeneratorProtocol {
    ...
    func generateQuestions(maxCount: Int, difficulty: QuizDifficulty?) -> [QuizQuestion] {
        ...
        let surplusForInvestment = netTakeHome * 0.25

        // 💡 Investment Strategy Question
        if shouldIncludeDifficulty(difficulty, .medium) && questions.count < maxCount {
            let question = createInvestmentStrategyQuestion(
                latestPayslip: latestPayslip,
                surplusForInvestment: surplusForInvestment
            )
        ...
        if shouldIncludeDifficulty(difficulty, .hard) && questions.count < maxCount {
            let question = createInflationProtectionQuestion(
                latestPayslip: latestPayslip, netTakeHome: netTakeHome
            )
            questions.append(question)
        }
        ...
        if shouldIncludeDifficulty(difficulty, .medium)
            ...
            && !latestPayslip.earnings.isEmpty {
            if let question = createHighestAllowanceQuestion(latestPayslip: latestPayslip) {
                questions.append(question)
            }
        }
    ...
    private func createInvestmentStrategyQuestion(
        latestPayslip: any PayslipProtocol,
        surplusForInvestment: Double
    ) -> QuizQuestion {
        let projectedWealth = surplusForInvestment * 12 * 15 * 1.12
        let contextData = QuizContextData(
            ...
            calculationDetails: [
                "investment_surplus": surplusForInvestment,
        ...
        let surplusStr = formatCurrency(surplusForInvestment)
        let wealthStr = formatCurrency(projectedWealth)
        let questionText = "If you invest ₹\(surplusStr) monthly in equity mutual funds, " +
            "you could have ₹\(wealthStr) in 15 years. The key factor is:"

        let explanation = "Compounding is wealth creation's most powerful force. " +
            "Your money grows exponentially because you earn returns on both principal " +
            "and all previous returns. Starting early and staying consistent maximizes this."
    ...
    private func createInflationProtectionQuestion(
        latestPayslip: any PayslipProtocol,
        netTakeHome: Double
    ) -> QuizQuestion {
        let contextData = QuizContextData(
        ...
        let questionText = "Inflation averages 6% annually. To maintain your " +
            "₹\(netStr) purchasing power, your investments must earn:"

        let explanation = "Inflation erodes money's purchasing power. " +
            "If inflation is 6% and your investment earns 4%, you lose 2% purchasing " +
            "power annually. Always target returns above inflation rate to build real wealth."
    ...
    private func createHighestAllowanceQuestion(
        latestPayslip: any PayslipProtocol
    ) -> QuizQuestion? {
        ...
        guard allowanceAmounts.count >= 2 else { return nil }
        ...
        let contextData = QuizContextData(
            ...
            comparisonPeriod: nil,
            specificMonth: latestPayslip.month,
            calculationDetails: ["highest_allowance": highestAllowance.1]
        )

        let amountStr = formatCurrency(highestAllowance.1)
        let explanation = "\(highestAllowance.0) of ₹\(amountStr) is your highest " +
            "earning component, contributing significantly to your total income."
