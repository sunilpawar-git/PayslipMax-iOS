...
struct PayslipNavigation {
    ...
    @MainActor
    static func detailView(for payslip: AnyPayslip) -> some View {
    ...
    @MainActor
    static func simplifiedDetailView(for payslip: AnyPayslip) -> some View {
        let viewModel = PayslipDetailViewModel(
            payslip: payslip,
            securityService: DIContainer.shared.securityService,
            dataService: DIContainer.shared.dataService
        ...
        Group {
            if let payslip = payslip {
                ...
                .navigationDestination(for: PayslipItem.self) { item in
                    // The PayslipItem conforms to PayslipProtocol, so we can pass it directly
                    detailView(for: item)
                }
